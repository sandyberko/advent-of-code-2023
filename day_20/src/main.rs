...
fn pulse_propogation(input: &str) -> usize {
    let (state, ids) = parse_modules(input);
    let initial_dest = [ids[BROADCASTER]];

    let pulses = (1..=1000)
        .map(|_i| {
            let mut queue = vec![(usize::MAX, Pulse::Low, &initial_dest as &[_])];
...
fn pulse_to_rx(input: &str) -> usize {
    let (state, ids) = parse_modules(input);
    let rx = ids["rx"];
    let inital_dest = [ids[BROADCASTER]];
        ...
        while !queue.is_empty() {
            ...
            for (src, pulse, dest) in queue {
                for dest in dest.iter() {
                    ...
                    if let Some((pulse, dests)) = self::pulse(&state, src, *dest, pulse) {
...
fn parse_modules(input: &str) -> (Vec<Module<'_>>, HashMap<&'_ str, usize>) {
